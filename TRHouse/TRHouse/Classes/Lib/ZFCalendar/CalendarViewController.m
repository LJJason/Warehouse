...
- (void)initView{
    ...
    self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-50-64) collectionViewLayout:layout]; //初始化网格视图大小
...
//    NSDateComponents *components = [[NSDate date] YMDComponents];
//    NSLog(@"%zd", components.day);
//    if (section == 0) {
//        NSLog(@"%zd" ,monthArray.count);
//        return monthArray.count - components.day;
//    }else {
//    
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader){
        ...
        monthHeader.masterLabel.text = [NSString stringWithFormat:@"%zd年 %zd月",model.year,model.month];//@"日期";
//        monthHeader.backgroundColor = [[UIColor blueColor] colorWithAlphaComponent:0.8f];
